...
class EmailServer:
    ...
    def __init__(self, host, port, address, user, password):
        self.host = host
        self.port = port
    ...
    def send_email(self,
        ...
        email_server.login(self.user, self.password)
        email_server.sendmail(msg['From'], msg['To'], msg.as_string())
        email_server.quit()

    def send_password_reset(self, to: str, token: str):
        self.send_email(to=to,
                        subject='PittGrub password reset request',
                        text=RESET_TEXT.format(token=token),
                        html=RESET_HTML.format(token=token))

    def send_verification(self, to: str, code: str):
        self.send_email(to=to,
